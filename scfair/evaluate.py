...
def ood_for_given_covs_1(
    ...
    r2_eval(adata, true_x_counts_mean, px_cf_mean_pred, n_top_deg=n_top_deg)
...
def r2_eval(adata, true_x_counts_mean, px_cf_mean_pred, n_top_deg: int = 100):
    adata.var['name'] = adata.var.index
    sc.tl.rank_genes_groups(adata, 'gender', method='wilcoxon', key_added="wilcoxon")
    ranked_genes = sc.get.rank_genes_groups_df(adata, group='Female', key='wilcoxon', gene_symbols='name')
